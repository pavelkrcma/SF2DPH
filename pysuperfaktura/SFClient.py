...
from pysuperfaktura.expense import SFExpense, SFExpenseClient
...
class SFClient:
    ...
    list_expenses_url = '/expenses/index.json'
    ...
    def list_expenses(self, params=None):
        ...
        if params:
            filter_url = self.construct_filter(params)
        returned_expenses = self.send_request(self.list_expenses_url, filter=filter_url)
        expense_list = []
        if len(returned_expenses) != 0:
            for returned_expense in returned_expenses:
                returned_items = []
                expense = SFExpense(None, returned_expense, returned_items)
